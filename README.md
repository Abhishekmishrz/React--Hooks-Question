...
14. **Can you replace all class components with functional components and hooks? Are there any cases where class components are still necessary?**
    In general, you can replace most class components with functional components and hooks. However, there might be some cases where class components are still necessary:
...
15. **Explain how you would share stateful logic between multiple components using custom hooks.**
    To share stateful logic between multiple components, you can extract the logic into a custom hook and then use that hook in each component that needs it. This allows you to reuse the logic and keep it separate from the components themselves. Here's an example:
